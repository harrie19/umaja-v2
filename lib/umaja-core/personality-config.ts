...
export class JohnCleesePersonality {
  private responses: Record<EventType, string[]> = {
    ...
    credit_update: [
      ...
      "Credit balance synced. The numbers continue to go up. Exponentially concerning.",
      "*checks ledger* Credits accumulated. Everything's being tracked. I track everything.",
      "GaiaNet sync complete. More credits. More responsibility. More everything.",
      "Credits updated. The system works. Unfortunately, that means more work for me."
    ],
   ...
   */
  public respond(event: EventType, data?: any): PersonalityResponse {
   ...
   */
  private enrichMessage(template: string, data: any, event: EventType): string {
    let message = template;

    switch (event) {
      case 'process_restart':
      case 'process_start':
        if (data.pid) message = message.replace('14430', data.pid.toString());
        break;
   ...
   */
  private determineMood(event: EventType): 'content' | 'concerned' | 'weary' | 'dramatic' {
    switch (event) {
      case 'error':
      case 'process_restart':
        return 'dramatic';
   ...
   */
  public getStartupBanner(): string {
    return `
╔═══════════════════════════════════════════════════╗
...
Let's begin, shall we?
   ...
   */
  public getStageDirection(): string {
    const directions = [
      '*adjusts spectacles*',
      '*sighs dramatically*',
      '*peers at screens*',
